...
import { Play, Heart, MessageCircle, Share2, MoreVertical, Search } from "lucide-react";
      ...
      <Tabs defaultValue="short" className="max-w-2xl mx-auto">
        ...
        <TabsContent value="short" className="mt-0 p-4 space-y-4">
          {shortVideos.map((video) => (
            <div key={video.id} className="glass rounded-3xl overflow-hidden border border-white/10" data-testid={`short-${video.id}`}>
              <div className="relative aspect-[9/16] max-h-[600px] overflow-hidden">
                ...
                <div className="absolute right-4 bottom-20 flex flex-col gap-4">
                  <button className="flex flex-col items-center gap-1" data-testid={`like-${video.id}`}>
                    <div className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center">
                      <Heart className="w-6 h-6 text-white" />
                    </div>
                    <span className="text-white text-xs font-semibold">{video.likes}</span>
                  </button>
                  <button className="flex flex-col items-center gap-1" data-testid={`comment-${video.id}`}>
                    <div className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center">
                      <MessageCircle className="w-6 h-6 text-white" />
                    </div>
                  </button>
                  <button className="flex flex-col items-center gap-1" data-testid={`share-${video.id}`}>
                    <div className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center">
        ...
        <TabsContent value="long" className="mt-0 p-4 space-y-4">
          {longVideos.map((video) => (
            <div key={video.id} className="glass rounded-2xl overflow-hidden border border-white/10" data-testid={`long-${video.id}`}>
